...
export class InitialCreateSchema1539095419272 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<any> {
        ...
        await queryRunner.query("CREATE TABLE `epn_role_session` (`session_number` int NOT NULL AUTO_INCREMENT, `epn_role_name` char(16) NOT NULL, `epn_hostname` varchar(255) NOT NULL, `n_subtimeframes` int NOT NULL, `bytes_in` int NOT NULL, `bytes_out` int NOT NULL, `session_start` datetime(0) NOT NULL, `session_end` datetime(0) NOT NULL, `fk_run_number` int NOT NULL, PRIMARY KEY (`session_number`, `epn_role_name`, `fk_run_number`)) ENGINE=InnoDB");
        ...
        await queryRunner.query("CREATE TABLE `run_quality_history` (`run_quality_history_id` int NOT NULL AUTO_INCREMENT, `change_time` datetime(0) NOT NULL, `runQuality` enum ('test') NOT NULL, `fk_run_number` int NOT NULL, `fk_changed_by_user_id` int NOT NULL, PRIMARY KEY (`run_quality_history_id`, `fk_run_number`)) ENGINE=InnoDB");
        await queryRunner.query("CREATE TABLE `run` (`run_number` int NOT NULL AUTO_INCREMENT, `time_o2_start` datetime(0) NOT NULL, `time_trg_start` datetime(0) NOT NULL, `time_trg_end` datetime(0) NOT NULL, `time_o2_end` datetime(0) NOT NULL, `activity_id` char(64) NOT NULL, `run_type` enum ('test') NOT NULL, `run_quality` enum ('test') NOT NULL, `n_detectors` int NOT NULL, `n_flps` int NOT NULL, `n_epns` int NOT NULL, `n_timeframes` int NOT NULL, `n_subtimeframes` int NOT NULL, `bytes_read_out` int NOT NULL, `bytes_timeframe_builder` int NOT NULL, PRIMARY KEY (`run_number`)) ENGINE=InnoDB");
        await queryRunner.query("CREATE TABLE `run_eor_history` (`eor_history_id` int NOT NULL AUTO_INCREMENT, `subsystem` enum ('test') NOT NULL, `change_time` datetime(0) NOT NULL, `end_of_run_reason` enum ('test') NOT NULL, `fk_run_number` int NOT NULL, `fk_changed_by_user_id` int NOT NULL, PRIMARY KEY (`eor_history_id`, `subsystem`, `fk_run_number`)) ENGINE=InnoDB");
        ...
        await queryRunner.query("CREATE TABLE `user` (`user_id` int NOT NULL AUTO_INCREMENT, `sams_id` int NOT NULL, `token` varchar(255) NULL, `token_valid_until` datetime(0) NULL, PRIMARY KEY (`user_id`)) ENGINE=InnoDB");
        await queryRunner.query("CREATE TABLE `log` (`log_id` int NOT NULL AUTO_INCREMENT, `subtype` enum ('run', 'subsystem', 'announcement', 'intervention', 'comment') NOT NULL, `origin` enum ('human', 'process') NOT NULL, `creation_time` datetime(0) NOT NULL, `title` varchar(255) NOT NULL, `text` varchar(255) NOT NULL, `subsystem_fk_subsystem_id` int NULL, `announcement_valid_until` datetime(0) NULL, `comment_fk_parent_log_id` int NULL, `comment_fk_root_log_id` int NULL, `fk_user_id` int NULL, PRIMARY KEY (`log_id`)) ENGINE=InnoDB");
        await queryRunner.query("CREATE TABLE `attachment` (`file_id` int NOT NULL AUTO_INCREMENT, `creation_time` datetime(0) NOT NULL, `title` varchar(255) NOT NULL, `file_mime` varchar(255) NOT NULL, `file_data` blob NOT NULL, `file_md5` char(16) NULL, `fk_log_id` int NOT NULL, PRIMARY KEY (`file_id`)) ENGINE=InnoDB");
        await queryRunner.query("CREATE TABLE `intervention_log` (`time_of_call` datetime(0) NULL, `intervention_type` enum ('test') NULL, `location` enum ('test') NULL, `action_taken` varchar(255) NULL, `log_id` int NOT NULL, UNIQUE INDEX `REL_e90cb19497046349b04ffd752c` (`log_id`), PRIMARY KEY (`log_id`)) ENGINE=InnoDB");
